...
summary(rawDF$Attrition)[1] / sum(summary(rawDF$Attrition))
...
```{r}
myGrid <- gbmFit2$bestTune

gbmFit3 <- train(Attrition ~ ., data = hrTrain, 
                 method = "gbm", 
                 trControl = fitControl,
                 ## This last option is actually one
                 ## for gbm() that passes through
...
```{r}
hrTest$predicted <- predict(gbmFit3, newdata = hrTest)
results <- table(hrTest$Attrition, hrTest$predicted)
accuracy <- sum(diag(results)) / dim(hrTest)[1]
accuracy
```
